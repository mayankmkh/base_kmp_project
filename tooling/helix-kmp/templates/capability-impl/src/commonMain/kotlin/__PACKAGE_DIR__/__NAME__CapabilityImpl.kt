...
// while the last attempt is younger than its interval), a retry on reconnect for observed keys
// whose last attempt failed offline, and the `status` behind every observation. `observations`
// wraps a durable value flow in `observing` and applies the contract's `SyncStatus.toOperation`
// mapping, so every Query goes through it and the appearance sync and the offline retry know about
// it. `:capability:posts-impl` is the worked example with SQLDelight, Ktor, and a second keyed
// coordinator for detail rows.
...
internal class __NAME__CapabilityImpl(
    ...
    private val sync = SyncCoordinator<Unit>(scope, sync = { _, _ -> synchronize() })

    init {
        scope.launch {
            connectivityMonitor.reconnects().collect { sync.retryOffline(RefreshQos.background()) }
        }
    }
